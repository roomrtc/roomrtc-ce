...
class Me extends React.Component {
    ...
    return (
      <div
        ...
        <PeerView
          ...
          audioRtpParameters={audioProducer
          ? audioProducer.rtpParameters
          : null}
          videoRtpParameters={videoProducer
          ? videoProducer.rtpParameters
          : null}
          audioTrack={audioProducer
          ? audioProducer.track
          : null}
          videoTrack={videoProducer
          ? videoProducer.track
          : null}
          videoVisible={videoVisible}
          audioCodec={audioProducer
          ? audioProducer.codec
          : null}
          videoCodec={videoProducer
          ? videoProducer.codec
          : null}
          audioScore={audioProducer
          ? audioProducer.score
          : null}
          videoScore={videoProducer
          ? videoProducer.score
          : null}
  ...
  componentDidMount() {
    ...
    setTimeout(() => {
      if (!this._mounted || (this.props as any).me.displayNameSet) {
        return;
      }
...
const mapStateToProps = (state) => {
  ...
  const audioProducer = producersArray.find((producer: any) => producer.track.kind === 'audio');
  const videoProducer = producersArray.find((producer: any) => producer.track.kind === 'video');
