...
class Room extends EventEmitter {
  ...
  private protooRoom: any;
  private broadcasters: any;
  private networkThrottled: any;
  private mediasoupRouter: any;
  private audioLevelObserver: any;
  private bot: any;
  ...
  constructor({
    ...
    this.plainRtpTransportOptions = plainRtpTransportOptions;
  ...
  logStatus() {
    logger.info(
      ...
      this.roomId,
      this.protooRoom.peers.length,
      this.mediasoupRouter._transports.size); // NOTE: Private API.
  }
  ...
  handleProtooConnection({ peerId, consume, protooWebSocketTransport }) {
    ...
    try {
      peer = this.protooRoom.createPeer(peerId, protooWebSocketTransport);
    } catch (error) {
    ...
    switch (type) {
      ...
      case 'plain': {
        const plainRtpTransportOptions = {
          ...this.plainRtpTransportOptions,
          rtcpMux: Boolean(rtcpMux),
          comedia: Boolean(comedia),
          multiSource: Boolean(multiSource),
        };

        const transport = await this.mediasoupRouter.createPlainRtpTransport(
          plainRtpTransportOptions);
  ...
  async _handleProtooRequest(peer, request, accept, reject) {
    switch (request.method) {
      ...
      case 'join': {
        ...
        const joinedPeers =
          [
            ...
            ...Array.from(this.broadcasters.values()),
          ];
      ...
      case 'createWebRtcTransport': {
        ...
        const { maxIncomingBitrate } = this.webRtcTransportOptions;
